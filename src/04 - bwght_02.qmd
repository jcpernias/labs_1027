...
::: {.callout-note appearance="simple"}
Cargue el paquete `car`. En esta sección usaremos la función `lht` para contrastar restricciones lineales sobre los parámetros del modelo estimado en la sección anterior.
:::
...
::: {.callout-note appearance="simple"}
Podemos contrastar la hipótesis de que varias pendientes son 0. Por ejemplo, podemos contrastar la significación conjunta de `lfaminc` y `second` ($H_0\!: \alpha_7 = \alpha_{10} = 0$) con las siguientes instrucciones:
...
::: {.callout-note appearance="simple"}
Si queremos contrastar la hipótesis de que las pendientes de `first` y de `second` son iguales ($H_0\!: \alpha_6 = \alpha_{7}$), reescribimos la hipótesis de forma que el lado derecho de las igualdades sea igual a $0$: $$
...
::: {.callout-note appearance="simple"}
Contraste la hipótesis nula de que el efecto del tabaco sobre el peso de los recién nacidos es independiente de cuantos cigarrillos se fumaron durante el embarazo (los parámetros $\alpha_1$ a $\alpha_5$ son iguales: $H_0\!: \alpha_1 = \alpha_{2} = \alpha_{3} = \alpha_{4} = \alpha_{5}$).
:::
...
::: {.callout-note appearance="simple"}
Contraste la hipótesis anterior conjuntamente con las hipótesis de no significación de `second` y `lfaminc`.
:::
...
::: {.callout-note appearance="simple"}
Estime el modelo restringido que se obtiene al imponer todas las restricciones del apartado anterior. Compare las estimaciones obtenidas del efecto del tabaco sobre el peso de los recién nacidos en este modelo y en el modelo de regresión simple estimado en la sección 2.
:::
