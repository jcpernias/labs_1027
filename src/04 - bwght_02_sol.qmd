...
```{r}
h0 <- c("cigs_1_5 = cigs_6_10",
        "cigs_1_5 = cigs_11_15",
        "cigs_1_5 = cigs_16_20", 
        "cigs_1_5 = cigs_over_20")
lht(mod2, h0)
```
...
*Respuesta:* El *p*-valor del contraste es mucho mayor que el nivel de significación $\alpha = 0.05$, por lo que no se puede rechazar la hipótesis nula. No hay evidencia en contra de las hipótesis contrastadas.
...
```{r}
mod2r <- lm(lbwght ~ smoker + first + white + male, 
            data = bwght)
summary(mod2r)
```

*Respuesta:* Las estimaciones de la pendiente de la variable `smoker` son muy similares en los dos modelos: los hijos de las madres que fumaron durante el embarazo pesaban al nacer un 7,7% menos que los hijos de madres no fumadoras.

En el modelo de regresión múltiple se tienen en cuenta más factores que pueden afectar al peso de los recién nacidos (orden de nacimiento, etnia, sexo, renta familiar), por lo que es más creíble en este caso la interpretación causal del efecto de `smoker` sobre la salud de los recién nacidos.
