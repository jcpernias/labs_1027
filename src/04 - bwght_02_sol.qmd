...
```{r}
mod2r <- lm(lbwght ~ smoker + first + second + 
              white + male + lfaminc, 
            data = bwght)
summary(mod2r)
```

*Respuesta:* Las estimaciones de la pendiente de la variable `smoker` son muy similares en los dos modelos: los hijos de las madres que fumaron durante el embarazo pesaban al nacer alrededor de un 7.5% menos que los hijos de madres no fumadoras.
