...
Un análisis gráfico informal apuntaba a la posible existencia de problemas de especificación. En esta sección ampliaremos el modelo de regresión para incluir otros factores que explicativos.
...
- $\mathit{prtystrA}$ es el porcentaje de votos que obtuvo el partido de A en las elecciones presidenciales más recientes. Con esta variable se trata de medir la fuerza del partido del candidato A en el distrito electoral. Esperamos que $\beta_5$ sea positivo, y los candidatos reciban más votos en aquellos distritos en que sus partidos sean más fuertes.
...
Las estimaciones MCO permitirían rechazar que los parámetros de cada una de las variables explicativas sean iguales a $0$ a un nivel de significación del $5\%$. 
Las estimaciones de los efectos de los gastos de campaña tienen magnitudes similares aunque de signo contrario: los resultados electorales 
del candidato A mejoran una tasa de $0{,}03$ puntos porcentuales por cada $1.000$ dólares adicionales de gastos en su campaña. 
La estimación del parámetro de `democA` indica que en estas elecciones los candidatos demócratas partieron con una ventaja a su favor de algo más de $9$ puntos porcentuales. El parámetro de `partystrA`  tiene el signo positivo como se esperaba, confirmando que los candidatos suelen obtener mejores resultados en aquellos distritos en que su partido es más fuerte. El ajuste es razonablemente bueno, y el modelo de regresión es capaz de explicar el $62{,6}\%$ de la variación de la variable dependiente. Finalmente, el contraste $F$ de la regresión toma el valor $F = 70{,}4$, llevando al rechazo al $5\%$ de significación de la hipótesis nula de que todas las pendientes son iguales a $0$ (valor-*p*  $< 0{,}001$).
...
$$
  H_1\!: \beta_2 \neq -\beta_1
$$
A continuación discutiremos tres enfoques para llevar a cabo el contraste. Aunque los tres son completamente equivalentes en este caso, a lo largo del curso encontraremos situaciones en las prefiramos uno de ellos sobre los otros dos. 
...
Podemos pensar en el contraste de Wald como una generalización del contraste $t$ que puede utilizarse con todo tipo de restricciones lineales sobre los parámetros del modelo. La deducción del contraste requiere el uso de álgebra matricial y no se abordará en este curso. Una de las ventajas del contraste de Wald es que *sólo requiere la estimación del modelo no restringido*. 

Para contrastar hipótesis lineales generales podemos usar la función `lht` del paquete `car` (este paquete no se instala por defecto; puede instalarse, si es necesario, a través del menú `Tools | Install packages...`).  
```{r}
library(car)
```

Siempre necesitamos especificar al menos dos argumentos con la función `lht`. El primer argumento indica el modelo de regresión sobre el que queremos contrastar la hipótesis nula (`mod3` en nuestro caso). El segundo argumento describe la hipótesis que queremos contrastar. Es conveniente reescribir la hipótesis nula como combinaciones lineales de parámetros que son iguales a 0. En nuestro caso, reescribimos la hipótesis:
$$
  H_0\!: \beta_2 = -\beta_1
$$
como
$$
  H_0\!: \beta_1 + \beta_2 = 0
$$
Creamos una variable `h0` donde especificamos el lado izquierdo de la hipótesis nula: 
```{r}
h0 <- "expendA + expendB"
```
La función `lht` interpretará la variable `h0` como una hipótesis donde la suma de los parámetros de las variables `expendA` y `expendB` son iguales a 0. En la expresión anterior son muy importantes las dobles comillas `"`: sin ellas, R trataría de sumar dos variables, lo que, generalmente, causaría un error.  

La siguiente instrucción calcula el contraste de Wald:
```{r}
lht(mod3, h0)
```
Al ejecutar esta instrucción se presenta prácticamente la misma información que obtuvimos con el contraste $F$. En concreto, el estadístico $F$ y su valor-*p* son idénticos a los que discutimos antes. Por tanto, igual que antes, no se rechazaría la hipótesis nula al $5\%$ de significación.

## Reparametrización del modelo

La tercera alternativa consiste en reparametrizar el modelo no restringido. Lo que pretendemos con esta estrategia es estimar directamente parámetros que expresen las restricciones de la hipótesis nula. En nuestro ejemplo queremos verificar que los efectos de los gastos de campaña de los candidatos son iguales pero con signos opuestos. Antes hemos visto que una forma de expresar esta hipótesis es:
$$
  H_0\!: \beta_1 + \beta_2 = 0
$$
...
Dado que los tres enfoques presentados son completamente equivalentes, se llega en todos los casos a la misma conclusión: no se encuentra evidencia en contra de la hipótesis de que los efectos de los gastos de campaña de los candidatos tengan la misma magnitud y signo opuestos. Al imponer esta hipótesis, obtenemos el modelo restringido:
$$
\mathit{voteA} = \beta_0 
  + \beta_1 \, \mathit{diffAB}
  + \beta_3 \, \mathit{democA}
