...
Antes de empezar a trabajar con RStudio es conveniente desactivar dos opciones que están activadas por defecto y que frecuentemente causan errores para los que no se encuentra una causa con facilidad. Con el menú `Tools | Global Options...` accedemos a un cuadro de diálogo. Debemos desactivar la opción 'Restore .Rdata into workspace at startup' y seleccionar la opción 'Never' en el desplegable 'Save workspace to .RData on exit:'. Después de realizar estos cambios el cuadro de diálogo debe quedar como muestra la siguiente figura:

```{r options-fig, out.width="50%", out.height="50%", fig.align='center', echo=FALSE}
...
Es preferible no trabajar con la consola e ir escribiendo las instrucciones que 
necesitemos en un archivo de texto (*script*). Para crear un nuevo script usamos el menú `File | New File > | R Script` o usamos el icono que aparece en la esquina superior izquierda de la ventana de RStudio:
...
Con R se pueden producir gráficos muy sofisticados. En este curso utilizaremos las funciones proporcionadas por el paquete `ggplot2`. A pesar de ser uno de los paquetes más populares no viene instalado por defecto en R y hay que instalarlo antes de poder usarlo. Una vez instalado, lo cargamos con `library`:
```{r}
library(ggplot2)
```

Podemos representar un gráfico de dispersión de los votos obtenidos por el candidato A, $\mathit{voteA}$, con respecto a sus gastos de campaña, $\mathit{expendA}$, utilizando la función `qplot` (*quick plot*). El primer argumento es la variable que se representará en el eje horizontal, el segundo argumento es la variable del eje vertical y con el argumento `data` indicamos en qué base de datos se encuentran esas variables:
```{r}
qplot(expendA, voteA, data = vote1)
```
...
```{r}
qplot(expendA, uhat1, data = vote1)
```
