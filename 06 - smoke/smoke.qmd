...
Para llevar a cabo el contraste con la función `lht` necesitamos un vector con los nombres de las variables cuyos parámetros son $0$ bajo la hipótesis nula. Una alternativa es crear ese vector manualmente:
```{r}
#| eval: false

names_slopes <- c("restaurn", "lincome", "lcigpric", "educ", "age")
```


Si hay muchas variables explicativas es fácil olvidar alguna o escribir incorrectamente su nombre. Alternativamente, podemos construir ese vector automáticamente con algunas funciones de R. En primer lugar, recuperamos las estimaciones de los parámetros con la función `coef`:
```{r}
# Recuperamos las estimaciones de los parámetros:
bhat <- coef(mlp)

# Obtenemos los nombres de los parámetros:
names_bhat <- names(bhat)

# Eliminamos el primer elemento del vector (la constante):
names_slopes <- names_bhat[-1]
```

Una vez guardado los nombres de las variables en la variable `names_slopes`, utilizamos `lht` para obtener el contraste de significación de la regresión robusto a heteroscedasticidad:
```{r}
