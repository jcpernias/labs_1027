...
# Preparativos
Utilizaremos los paquetes `sandwich` y `lmtest` para obtener errores típicos robustos a heteroscedasticidad y el paquete `mosaic` para hacer gráficos.
...
Guardamos los residuos en la variable `uhat1`:
```{r}
uhat1 <- resid(mod1)
```
...
```{r}
gf_point(uhat1 ~ bdrms, uhat1, data = hprice1)
```
