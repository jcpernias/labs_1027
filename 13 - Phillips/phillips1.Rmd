...
$$
\pi_t - \pi^e_t= \alpha + \beta (u_t - u^n) + \epsilon_t
$$
...
$$
\pi_t - \pi_{t-1}= \alpha + \beta (u_t - u^n) + \epsilon_t
$$
Podemos estimar el parámetro $\beta$ con el modelo de regresión:
$$
\Delta\pi_t = \delta + \beta u_t + \epsilon_t \tag{Mod2}
$$
donde el parámetro $\delta$ es igual a $\alpha - \beta u^n$.

## Datos

El archivo `phillips.csv` contiene series anuales referidas a la ecoomía española y que cubren el periodo que va desde 1963 hasta 2022. En este laboratorio usaremos las series:
