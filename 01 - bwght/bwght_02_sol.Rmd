...
1. Obtenga las estimaciones MCO de los parámetros del modelo:
  ...
  Guarde los resultados de la estimación del modelo en la variable `mod1`. Interprete las estimaciones obtenidas. 
    ...
    ```
    De acuerdo con estas esimaciones, los hijos de las madres que 
    fumaron durante el embarazo fue un 7,7% inferior al de los hijos 
    cuyas madres no fumaron. Este efecto es significativo a un nivel de 
    significación del 5%.
    ```
...
1. Estime por MCO los parámetros del modelo de regresión:
  ...
  Guarde los resultados de la estimación en la variable `mod2` e interprete los resultados.
    ...
    ```
    ```
    La estimaciones parecen indicar que el efecto del tabaco sobre 
    el peso de los recién nacidos es mayor, en valor absoluto, cuanto
    más fumaron las madres. Excepto la variable cigs_1_5, todas las 
    variables que miden el consumo de tabaco son significativas al 5%. 
    Por otro lado, la estimación del parámetro de la variable first 
    muestra que las madres primerizas tienen hijos que pesan un 4% 
    menos, la pendiente de white indicaría que los recién nacidos 
    blancos pesarían casi un 6% más que los bebés de otras etnias, y la 
    estimación del parámetro de male indicaría que los niños pesan al 
    nacer un 2,5% más que las niñas. Finalmente, las variables second y 
    lfaminc no tendrían efectos significativos al 5%.
    ```
...
2. Podemos contrastar la hipótesis de que varias pendientes son 0. Por ejemplo, podemos contrastar la significación conjunta de `lfaminc` y `second` ($H_0\!: \alpha_7 = \alpha_{10} = 0$) con las siguientes instrucciones:
  ...
  ¿Se rechaza la hipótesis nula a un nivel de significación del 5%?
    ...
    ```
    ```
    El valor del restadistico F es 1,16, por lo que no se rechaza 
    la hipótesis nula al 5% (valor-p = 0,31).
    ```
...
4. Si queremos contrastar la hipótesis de que las pendientes de `first` y de `second` son iguales ($H_0\!: \alpha_6 = \alpha_{7}$), reescribimos la hipótesis de forma que el lado derecho de las igualdades sea igual a 0: $H_0\!: \alpha_6 - \alpha_{7} = 0$. Las siguientes instrucciones realizan el contraste:
  ...
  ¿Se rechaza la hipótesis nula a un nivel de significación del 5%?
    ...
    ```
    ```
    El valor del restadistico F es 6,37, por lo que se rechaza 
    la hipótesis nula al 5% (valor-p = 0,01).
    ```
...
3. Contraste la hipótesis nula de que el efecto del tabaco sobre el peso de los recién nacidos es independiente de cuantos cigarrillos se fumaron durante el embarazo (los parámetros $\alpha_1$ a $\alpha_5$ son iguales: $H_0\!: \alpha_1 = \alpha_{2} = \alpha_{3} = \alpha_{4} = \alpha_{5}$).
    ...
    ```
    ```
    El valor del restadistico F es 0,87, por lo que no se rechaza 
    la hipótesis nula al 5% (valor-p = 0,48).
    ```
...
4. Contraste la hipótesis del apartado anterior conjuntamente con las hipótesis de no significación de `second` y `lfaminc`.
    ...
    ```
    ```
    El valor del restadistico F es 0,97, por lo que no se rechaza 
    la hipótesis nula al 5% (valor-p = 0,44).
    ```
...
5. Estime el modelo restringido que se obtiene al imponer todas las restricciones del apartado anterior. Compare las estimaciones obtenidas del efecto del tabaco sobre el peso de los recién nacidos en este modelo y en el modelo de regresión simple estimado en la sección 2.
    ...
    ```
    Si son iguales los parámetros que acompañan a las ficticias que 
    miden la intensidad del consumo de tabaco, sólo importaría el hecho 
    de si se fuma y no sería relevante cuánto se fuma. Por tanto, en el
    modelo restringido smoker reemplazaría a las variables cigs_X_Y. Por
    otro lado, las variables second y lfaminc no entrarían en el modelo
    restringido.
    ```
    ...
    ```
     ...
     ```
    La esimación del efecto del tabaco sobre el peso de los recién 
    nacidos obtenida con este modelo es similar a la que se obtuvo 
    con el modelo de regresión simple: el peso de los hijos de las 
    madres que fumaron durante el embarazo fue un 7,7% inferior al 
    de los hijos cuyas madres no fumaron. Este efecto es significativo 
